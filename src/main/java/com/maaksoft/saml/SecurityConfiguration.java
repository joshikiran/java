...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(final HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/saml*").permitAll().anyRequest().authenticated().and().apply(saml())
				...
				.keyname(this.keyAlias).keyPassword(this.password).and().protocol(this.protocol)
				.hostname(String.format("%s:%s", this.hostname, this.port)).basePath(this.contextPath).and()
